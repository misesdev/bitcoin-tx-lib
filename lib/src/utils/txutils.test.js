...
describe("addressToScriptPubKey", () => {
    ...
    test("P2PKH mainnet address (starts with 1) → valid P2PKH scriptPubKey", () => {
        ...
        const script = (0, txutils_1.addressToScriptPubKey)(address);
    ...
    test("P2SH address is not supported — throws", () => {
        expect(() => (0, txutils_1.addressToScriptPubKey)("2NBFNJTktNa7GZusGbDbGKRZTxdK9VVez3n")).toThrow();
    });
    test("completely invalid address throws", () => {
        expect(() => (0, txutils_1.addressToScriptPubKey)("abc123")).toThrow("not supported format address");
    });
    test("empty string throws", () => {
        expect(() => (0, txutils_1.addressToScriptPubKey)("")).toThrow();
    });
});
describe("pubkeyToScriptCode", () => {
    test("known public key → correct P2WPKH scriptCode (hex)", () => {
        ...
        expect((0, txutils_1.pubkeyToScriptCode)(publicKey)).toBe("1976a914a8439c50793b033df810de257b313144a8f7edc988ac");
    });
...
describe("scriptPubkeyToScriptCode", () => {
    ...
    test("P2WSH scriptPubkey (OP_0 + 32-byte hash) → passes through with length prefix", () => {
        ...
        const result = (0, txutils_1.scriptPubkeyToScriptCode)(p2wshHex);
        // First byte = total script length = 34 (2 + 32)
        expect(result[0]).toBe(34);
        // Remaining bytes = original scriptPubkey
        expect((0, index_1.bytesToHex)(result.slice(1))).toBe(p2wshHex);
    });
    test("P2PKH scriptPubkey passed to scriptPubkeyToScriptCode throws (not a SegWit script)", () => {
        ...
        expect(() => (0, txutils_1.scriptPubkeyToScriptCode)(p2pkhHex)).toThrow("scriptPubkey no segwit");
    });
    test("random/unknown script type throws", () => {
        expect(() => (0, txutils_1.scriptPubkeyToScriptCode)("deadbeef")).toThrow("scriptPubkey no segwit");
    });
});
