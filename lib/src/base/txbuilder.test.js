...
class TestBuilder extends txbuilder_1.TransactionBuilder {
    build(params, format) { return this.buildAndSign(params, format); }
    testScriptSig(input, params) { return this.generateScriptSig(input, params); }
    testWitness(input, params) { return this.generateWitness(input, params); }
    testValidateInput(input, inputs) { return this.validateInput(input, inputs); }
    testValidateOutput(output, outputs) { return this.validateOutput(output, outputs); }
    testOutputsRaw(outputs) { return this.outputsRaw(outputs); }
}
describe("TransactionBuilder", () => {
    ...
    describe("buildAndSign", () => {
        ...
        test("legacy transaction: raw equals txid build (no witness data)", () => {
            const params = Object.assign(Object.assign({}, segwitParams), { inputs: [legacyInput] });
            const raw = builder.build(params);
            const txid = builder.build(params, "txid");
            // For legacy, raw and txid serialization are equal
            expect(raw.length).toBe(txid.length);
        });
    });
    ...
    describe("validateInput", () => {
        ...
        test("odd-length txid throws (not valid hex)", () => {
            const bad = Object.assign(Object.assign({}, segwitInput), { txid: "a".repeat(63) });
            expect(() => builder.testValidateInput(bad, [])).toThrow();
        });
        test("txid with wrong byte count throws", () => {
            const bad = Object.assign(Object.assign({}, segwitInput), { txid: "abcd" });
            expect(() => builder.testValidateInput(bad, [])).toThrow("Expected a valid txid");
        });
        test("odd-length scriptPubKey throws", () => {
            const bad = Object.assign(Object.assign({}, segwitInput), { scriptPubKey: "001" });
        ...
        test("same txid + different vout → ALLOWED (valid multi-UTXO from same tx)", () => {
            const input1 = Object.assign(Object.assign({}, segwitInput), { vout: 1 });
            expect(() => builder.testValidateInput(input1, [segwitInput])).not.toThrow();
        });
        test("different txid + same vout → ALLOWED", () => {
            const input1 = Object.assign(Object.assign({}, segwitInput), { txid: "f".repeat(64) });
            expect(() => builder.testValidateInput(input1, [segwitInput])).not.toThrow();
        });
        test("three inputs: two from same txid (different vout) + one unrelated are all valid", () => {
            const base = { txid: "cc".repeat(32), vout: 0, scriptPubKey: "0014" + "ab".repeat(20), value: 10000 };
            const vout1 = Object.assign(Object.assign({}, base), { vout: 1 });
            const other = Object.assign(Object.assign({}, base), { txid: "dd".repeat(32) });
    ...
    describe("validateOutput", () => {
        ...
        test("amount = 0 throws", () => {
            expect(() => builder.testValidateOutput(Object.assign(Object.assign({}, baseOutput), { amount: 0 }), [])).toThrow("valid amount");
        });
        test("negative amount throws", () => {
            expect(() => builder.testValidateOutput(Object.assign(Object.assign({}, baseOutput), { amount: -1 }), [])).toThrow("valid amount");
        });
        test("invalid address throws", () => {
            expect(() => builder.testValidateOutput(Object.assign(Object.assign({}, baseOutput), { address: "bad" }), [])).toThrow("valid address");
        });
        test("empty address throws", () => {
            expect(() => builder.testValidateOutput(Object.assign(Object.assign({}, baseOutput), { address: "" }), [])).toThrow();
        });
    ...
    describe("outputsRaw", () => {
        ...
        test("two outputs produce more bytes than one", () => {
            const addr2 = new ecpairkey_1.ECPairKey().getAddress();
            const out2 = { address: addr2, amount: 1000 };
