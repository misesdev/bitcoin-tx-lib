...
const bip44TestnetHdk = () => _1.HDKManager.fromMnemonic(MNEMONIC, undefined, { purpose: 44, network: "testnet" });
const bip84MainnetHdk = () => _1.HDKManager.fromMnemonic(MNEMONIC, undefined, { purpose: 84, network: "mainnet" });
const bip84TestnetHdk = () => _1.HDKManager.fromMnemonic(MNEMONIC, undefined, { purpose: 84, network: "testnet" });
describe("HDKManager", () => {
    ...
    describe("fromXPub — all supported formats", () => {
        ...
        test("tpub master public key matches tprv master public key", () => {
            ...
            const full = _1.HDKManager.fromXPriv(TPRV);
            const watchOnly = _1.HDKManager.fromXPub(TPUB);
        ...
        test("zpub master public key matches zprv master public key", () => {
            const full = _1.HDKManager.fromXPriv(ZPRV);
            const watchOnly = _1.HDKManager.fromXPub(ZPUB);
            expect(watchOnly.getMasterPublicKey()).toEqual(full.getMasterPublicKey());
        });
        test("throws for unrecognized key prefix", () => {
            expect(() => _1.HDKManager.fromXPub("aaaa1234")).toThrow('Unrecognized extended key prefix: "aaaa"');
        });
    });
    ...
    describe("purpose and network inference from extended key prefix", () => {
        test("xprv → purpose 44, network mainnet", () => {
            const hdk = _1.HDKManager.fromXPriv(bip44Hdk().getXPriv());
            expect(hdk.purpose).toBe(44);
            expect(hdk.network).toBe("mainnet");
        });
        ...
        test("pathParams can override inferred purpose and network", () => {
            const hdk = _1.HDKManager.fromXPriv(TPRV, { purpose: 84, network: "mainnet" });
            expect(hdk.purpose).toBe(84);
            expect(hdk.network).toBe("mainnet");
        });
    });
